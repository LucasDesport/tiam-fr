...
The commercial sector encompasses seven [energy demands](demands/index.md) including space heating (CSC), space cooling (CSH), cooking (CCK), water heating (CHW), lighting (CLA), office equipments (COE), and other demands (COE). For each demand, the energy consumption in the base year is calibrated based on IEA energy balances (2020) but no additional constraints are applied so the commercial sector is free to employ different energies to satisfy the demand. The technologies and energies available are described in the following subsections.
